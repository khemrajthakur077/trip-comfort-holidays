        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-10">
            <header className="space-y-5">
              ...
              <h1 className="text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] break-words">
                {tourData.title}
              </h1>
              {tourData.subtitle && <p className="text-xl text-slate-500 font-medium max-w-2xl leading-relaxed break-words">{tourData.subtitle}</p>}
              ...
              <div className="space-y-12 relative before:content-[''] before:absolute before:left-4 before:top-2 before:bottom-2 before:w-[2px] before:bg-indigo-50">
                {Array.isArray(tourData.itinerary) ? tourData.itinerary.map((item, index) => (
                  <div key={index} className="relative pl-12 group">
                    ...
                    {/* FIXED: Box ke andar whitespace-pre-wrap add kiya hai */}
                    <div className="bg-slate-50/50 p-8 rounded-[2.5rem] border-2 border-transparent group-hover:border-indigo-50 group-hover:bg-white transition-all shadow-sm">
                      <h3 className="text-2xl font-black text-slate-800 mb-3 break-words">{item.title || item.day}</h3>
                      {item.highlight && <div className="inline-block bg-orange-50 text-orange-700 text-xs font-bold px-3 py-1 rounded-md mb-4 uppercase tracking-wider italic font-sans break-words">⭐ {item.highlight}</div>}
                      
                      {/* FIXED: Spots list automatic niche wali line mein jayegi */}
                      <ul className="text-slate-600 space-y-2">
                        {item.spots && Array.isArray(item.spots) ? item.spots.map((spot, i) => (
                          <li key={i} className="flex items-start gap-3 text-base leading-relaxed break-words whitespace-pre-wrap">
                             <span className="shrink-0">•</span> 
                             <span className="w-full">{spot}</span>
                          </li>
                        )) : item.stay && <li className="text-base font-medium flex items-start gap-2 break-words whitespace-pre-wrap">🏨 Stay: <span className="text-indigo-600">{item.stay}</span></li>}
                      </ul>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-indigo-50/30 p-10 rounded-[2.5rem] border border-indigo-50">
                  ...
                  <p className="text-slate-600 text-sm leading-[1.8] font-medium whitespace-pre-wrap break-words">{tourData.inclusions || 'Please contact for details'}</p>
                </div>
                <div className="bg-rose-50/30 p-10 rounded-[2.5rem] border border-rose-50">
                  ...
                  <p className="text-slate-600 text-sm leading-[1.8] font-medium whitespace-pre-wrap break-words">{tourData.exclusions || 'Please contact for details'}</p>
                </div>
            </div>
          </div>
